...
import { buildEffectivePrompt, buildSystemPrompt } from "./context";
import type {
	AIMessage,
	AIProvider,
...
import { registerProviderFactory } from "./provider";

// Re-export mapPiEvent from shared (runtime-agnostic)
export { mapPiEvent } from "./pi-events";
...
class PiSDKNodeSession extends BaseSession {
	...
	async *query(prompt: string): AsyncIterable<AIMessage> {
		const { mapPiEvent } = await import("./pi-events");
