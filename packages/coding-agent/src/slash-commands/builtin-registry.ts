...
import { handleMtuicontrol } from "../extensibility/extensions/m-mtuicontrol-extension";
import {
	buildConsolidateFn,
	executeMemoryBuild,
...
import { showMPanel, appendCustomResult } from "../utils/m-utils";
...
const BUILTIN_SLASH_COMMAND_REGISTRY: ReadonlyArray<SlashCommandSpec> = [
...

	{
		...
		handleTui: mpruneHandler,
	},

	{
		name: "mtuicontrol",
		description: "Manage headless TUI child sessions (spawn, prompt, keypress, command, wait, list, stop)",
		inlineHint: "<spawn|prompt|keypress|command|wait|list|stop> [args]",
		allowArgs: true,
		handleTui: async (command, runtime) => {
			if (!runtime.ctx.settings.get("mtuicontrol.enabled" as SettingPath)) {
				runtime.ctx.showWarning("mtuicontrol: not enabled — set mtuicontrol.enabled: true in config.");
				return;
			}
			runtime.ctx.editor.setText("");
			let result: string;
