...
Prerequisites:
    ...
    - Windows: Visual Studio Build Tools (CMake/Ninja auto-detected via vswhere;
      upstream `host` pseudo-target delegates to local N-API path on win32)
...
def main():
    ...
    parser.add_argument("--force-natives", action="store_true", help="Force Rust native rebuild (ignore Cargo.lock snapshot)")
    ...
    if args.force_natives:
        need_rust_build = True
        print("-> Forcing Rust compilation (--force-natives)")
    ...
    if need_rust_build and not args.dry_run:
        build_natives()
        # Record the lock state only when the rebuild actually ran. --skip-natives
        # must NOT snapshot, or a genuinely-needed rebuild is masked next run.
